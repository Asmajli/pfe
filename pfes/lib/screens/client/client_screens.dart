...
class _HomeState extends ConsumerState<HomeScreen> {
  ...
  Future<void> _fetchSuggestions(String q) async {
    ...
    // 1. ابحث في zones Firestore
    final allZones = ref.read(zonesProvider).asData?.value ?? [];
    final matchedZones = allZones.where((z) =>
      z.name.toLowerCase().contains(q.toLowerCase()) ||
      z.address.toLowerCase().contains(q.toLowerCase())
    ).toList();

    // 2. ابحث في OpenStreetMap
    try {
      ...
      try {
        res = await http.get(
          Uri.parse('https://nominatim.openstreetmap.org/search?q=$encoded&format=json&limit=4&accept-language=fr'),
          headers: {'User-Agent': 'ParkApp/1.0'},
        ).timeout(const Duration(seconds: 6));
      } catch (_) {
        apiType = 'photon';
        res = await http.get(
          Uri.parse('https://photon.komoot.io/api/?q=$encoded&limit=4&lang=fr'),
        ).timeout(const Duration(seconds: 8));
      }

      if (!mounted) return;

      final List locationResults = [];
      if (res.statusCode == 200) {
        final raw = jsonDecode(res.body);
        final List items = apiType == 'nominatim' ? (raw as List) : (raw['features'] as List? ?? []);
        for (final item in items) {
          if (apiType == 'nominatim') {
            locationResults.add(_SearchResult(
      ...
      // 3. دمج: zones أولاً ثم مواقع
      final zoneResults = matchedZones.map((z) {
        final latLng = _zoneLatLng(z);
  ...
  void _selectSuggestion(_SearchResult r) {
    ...
    _mapCtrl.move(ll.LatLng(r.lat, r.lng), r.isZone ? 17 : 15);
    if (r.isZone && r.zone != null) {
      setState(() => _selectedZone = r.zone);
      _sheetCtrl.animateTo(0.52,
              ...
              child: Padding(
                ...
                child: Row(children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      color: r.isZone
                          ? AppColors.blue2.withOpacity(0.12)
                          : AppColors.textMuted.withOpacity(0.08),
                      borderRadius: BorderRadius.circular(8),
                    ),
                    child: Center(child: Icon(
                      r.isZone ? Icons.local_parking : Icons.location_on_outlined,
                      color: r.isZone ? AppColors.blue2 : AppColors.textMuted,
                      size: 16,
                    )),
                  ),
                    ...
                    children: [
                      Text(r.description,
                        style: TextStyle(fontSize: 13,
                          fontWeight: r.isZone ? FontWeight.w700 : FontWeight.w400),
                        maxLines: 1, overflow: TextOverflow.ellipsis),
                      if (r.isZone && r.zone != null) ...[
                        const SizedBox(height: 2),
                        Row(children: [
                          StatusBadge(
                            label: r.zone!.isFull ? 'Complet' : '${r.zone!.freeSpots} libres',
                            color: r.zone!.isFull ? AppColors.red : AppColors.green),
                          const SizedBox(width: 6),
                          Text('${r.zone!.pricePerHour.toInt()} DT/h',
                  ...
                  if (r.isZone)
                    const Icon(Icons.arrow_forward_ios, size: 12, color: AppColors.blue2),
                ]),
              ),
            )).toList(),
          ),
  ...
  String _distance(ParkingZone z) {
    ...
    final dist = const ll.Distance().distance(ll.LatLng(_userLat!, _userLng!), target);
    return dist < 1000 ? '${dist.toInt()} m' : '${(dist / 1000).toStringAsFixed(1)} km';
  }

  Color _markerColor(ParkingZone z) => !z.isOpen || z.isFull
      ? AppColors.red : z.freeSpots <= 5 ? AppColors.yellow : AppColors.green;
      ...
      body: SafeArea(child: Stack(children: [
        ...
        FlutterMap(
          ...
          children: [
            ...
            MarkerLayer(markers: zones.map((z) {
              final color = _markerColor(z);
              return Marker(
                point: _zoneLatLng(z), width: 44, height: 52,
                ...
                child: Row(children: [
                  ...
                  Expanded(child: TextField(
                    ...
                    decoration: const InputDecoration(
                      hintText: 'Rechercher parking ou lieu...',
...
// ══════════════════════════════════════════════════════
//  SEARCH RESULT — avec support zones
// ══════════════════════════════════════════════════════
class _SearchResult {
...
class _BookState extends ConsumerState<BookingScreen> {
  ...
  Future<void> _book(AppUser user, ParkingZone zone) async {
    ...
    try {
      ...
      final confirmed = await showDialog<bool>(
        ...
        builder: (_) => Dialog(
          ...
          child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            const Text('Confirmer le paiement', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w800)),
            const SizedBox(height: 8),
            Text('${total.toStringAsFixed(2)} DT',
                style: const TextStyle(fontSize: 28, fontWeight: FontWeight.w900, color: AppColors.cyan)),
            const SizedBox(height: 4),
            Text('$_selectedSpotType · ${zone.name}', style: const TextStyle(fontSize: 12, color: AppColors.textMuted)),
            const SizedBox(height: 4),
            Text('${_formatDuration(_end.difference(_start))} · ${DateFormat('HH:mm').format(_start)} → ${DateFormat('HH:mm').format(_end)}',
                style: const TextStyle(fontSize: 11, color: AppColors.textMuted)),
            const SizedBox(height: 20),
            Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
              decoration: BoxDecoration(color: AppColors.yellow.withOpacity(0.07), borderRadius: BorderRadius.circular(10),
                border: Border.all(color: AppColors.yellow.withOpacity(0.25))),
              child: const Row(mainAxisSize: MainAxisSize.min, children: [
                Icon(Icons.info_outline, color: AppColors.yellow, size: 14), SizedBox(width: 6),
                Text('4242 4242 4242 4242 · 12/34 · 123', style: TextStyle(fontSize: 11, color: AppColors.yellow)),
              ])),
            const SizedBox(height: 20),
            Row(children: [
              Expanded(child: GestureDetector(onTap: () => Navigator.pop(_, false),
                child: Container(padding: const EdgeInsets.symmetric(vertical: 12),
                  decoration: BoxDecoration(color: AppColors.border, borderRadius: BorderRadius.circular(12)),
                  child: const Center(child: Text('Annuler', style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.textMuted)))))),
              const SizedBox(width: 12),
      ...
      if (confirmed != true) { setState(() => _loading = false); return; }
  ...
  void _showConflictDialog(String title, String message, {required bool isWarning}) {
    showDialog(
      context: context,
      builder: (_) => Dialog(
        backgroundColor: const Color(0xFF1a2235),
        ...
        child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: [
          Container(width: 64, height: 64,
            decoration: BoxDecoration(
              color: (isWarning ? AppColors.yellow : AppColors.red).withOpacity(0.15),
              shape: BoxShape.circle,
              border: Border.all(color: (isWarning ? AppColors.yellow : AppColors.red).withOpacity(0.4))),
            child: Icon(
              isWarning ? Icons.warning_amber_rounded : Icons.block,
              color: isWarning ? AppColors.yellow : AppColors.red, size: 32)),
          const SizedBox(height: 16),
          Text(title, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w800), textAlign: TextAlign.center),
          const SizedBox(height: 10),
          Text(message, style: const TextStyle(fontSize: 13, color: AppColors.textMuted, height: 1.5), textAlign: TextAlign.center),
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(12),
                border: Border.all(color: AppColors.blue2.withOpacity(0.4))),
              child: const Center(child: Text('Modifier le créneau',
                  style: TextStyle(color: AppColors.blue2, fontWeight: FontWeight.w700))))),
        ])),
      ),
    );
  }
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const Text('Présentez ce QR à l\'entrée du parking',
          ...
          const SizedBox(height: 20),
          ParkButton(label: 'Voir mes réservations', icon: Icons.bookmark_outline,
            onTap: () { Navigator.pop(context); context.go('/client/reservations'); }),
          const SizedBox(height: 10),
          GestureDetector(onTap: () { Navigator.pop(context); context.pop(); },
            child: const Text("Retour à l'accueil", style: TextStyle(fontSize: 13, color: AppColors.textMuted))),
        ]),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final zone = zonesAsync.asData?.value.firstWhere(
      ...
      orElse: () => const ParkingZone(id: '', name: '...', type: '', address: '',
    ...
    return Scaffold(
      ...
      body: SafeArea(child: ListView(padding: const EdgeInsets.all(20), children: [
        if (zone != null && zone.id.isNotEmpty) ...[
          ...
          const SizedBox(height: 16),
          const Text('TYPE DE PLACE', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: AppColors.textMuted, letterSpacing: 0.8)),
          const SizedBox(height: 10),
          SingleChildScrollView(scrollDirection: Axis.horizontal,
            child: Row(children: _spotTypes.map((t) {
              final color = Color(t['colorVal'] as int);
              final isSelected = _selectedSpotType == t['label'];
              return GestureDetector(
                onTap: () => setState(() { _selectedSpotType = t['label'] as String; _selectedSpotPrice = t['price'] as double; }),
                child: AnimatedContainer(duration: const Duration(milliseconds: 200),
                  ...
                  child: Column(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    Text('${(t['price'] as double).toStringAsFixed(1)} DT/h', style: TextStyle(fontSize: 10, color: color)),
          ...
          ParkCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              GestureDetector(
                ...
                child: Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    const Icon(Icons.edit_calendar_outlined, color: AppColors.green, size: 14), const SizedBox(width: 6),
            ...
            Row(children: [
              ...
              GestureDetector(
                ...
                child: Container(padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
                  ...
                  child: Row(mainAxisSize: MainAxisSize.min, children: [
                    const Icon(Icons.edit_calendar_outlined, color: AppColors.red, size: 14), const SizedBox(width: 6),
            ...
            const SizedBox(height: 10),
            Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              const Icon(Icons.schedule, color: AppColors.cyan, size: 15), const SizedBox(width: 6),
              const Text('Durée : ', style: TextStyle(fontSize: 12, color: AppColors.textMuted)),
              Text(_formatDuration(_end.difference(_start)),
          ...
          ParkCard(bgColor: AppColors.blue.withOpacity(0.07), borderColor: AppColors.blue2.withOpacity(0.25),
            child: Column(children: [
              ...
              const Divider(height: 20, color: AppColors.border),
              Row(children: [
                const Text('Total', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700)), const Spacer(),
...
class _ReservationsScreenState extends ConsumerState<ReservationsScreen> {
  ...
  // ── In-app banner ──
  String? _bannerMsg;
  Color _bannerColor = AppColors.yellow;
  Timer? _bannerTimer;

  void _showBanner(String msg, Color color) {
    _bannerTimer?.cancel();
    setState(() { _bannerMsg = msg; _bannerColor = color; });
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      if (user != null) {
        ...
        // Auto-annule les réservations avec +30min de retard d'entrée
        await ref.read(parkingServiceProvider).autoExpireNoShows(user.uid);
      }
    });
  ...
  void _checkReminders() {
    final resvs = ref.read(userReservationsProvider(
      ref.read(currentUserProvider).asData?.value?.uid ?? '')).asData?.value ?? [];
    final now = DateTime.now();
    ...
    showDialog(context: context, barrierDismissible: false,
      builder: (dialogContext) => Dialog(
        ...
        child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          const SizedBox(height: 6),
          Text('Place ${r.spotNumber} · Fin : ${DateFormat('HH:mm').format(r.endTime!)}',
              style: const TextStyle(fontSize: 12, color: AppColors.cyan, fontWeight: FontWeight.w600), textAlign: TextAlign.center),
          const SizedBox(height: 20),
          const Text('PROLONGER DE :', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: AppColors.textMuted, letterSpacing: 0.8)),
          const SizedBox(height: 10),
          Row(children: [
            ...[{'label': '+30 min', 'minutes': 30}, {'label': '+1h', 'minutes': 60}, {'label': '+2h', 'minutes': 120}].map((opt) {
              ...
              return Expanded(child: Padding(padding: const EdgeInsets.symmetric(horizontal: 4),
                child: GestureDetector(
                  onTap: () async {
                    ...
                    try {
                      final newEnd = await ref.read(prolongationServiceProvider).prolonger(
                        reservationId: r.id, extraMinutes: mins, pricePerHour: r.pricePerHour);
                      final updatedR = Reservation(id: r.id, userId: r.userId, userName: r.userName,
                        zoneId: r.zoneId, zoneName: r.zoneName, spotNumber: r.spotNumber,
                  ...
                  child: Container(padding: const EdgeInsets.symmetric(vertical: 10),
                    decoration: BoxDecoration(color: AppColors.blue2.withOpacity(0.08), borderRadius: BorderRadius.circular(10),
                      border: Border.all(color: AppColors.blue2.withOpacity(0.25))),
                    child: Column(children: [
                      Text(label, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700)),
                      const SizedBox(height: 2),
                      Text('+${cost.toStringAsFixed(2)} DT', style: const TextStyle(fontSize: 10, color: AppColors.textMuted)),
                    ])),
                )));
            }),
          ]),
          const SizedBox(height: 16),
          GestureDetector(onTap: () => Navigator.of(dialogContext).pop(),
            child: const Text('Non merci, je vais partir', style: TextStyle(fontSize: 12, color: AppColors.textMuted))),
        ])),
      ),
    );
  }
      ...
      body: Column(children: [
        ...
        if (_bannerMsg != null)
          AnimatedContainer(
            ...
            color: _bannerColor.withOpacity(0.92),
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
            child: SafeArea(
              bottom: false,
              child: Row(children: [
                Icon(
                  _bannerColor == AppColors.red
                      ? Icons.warning_rounded
                      : Icons.timer_outlined,
                  color: Colors.white, size: 20),
                const SizedBox(width: 10),
                Expanded(child: Text(_bannerMsg!,
                    style: const TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w700))),
                GestureDetector(
                  onTap: () { _bannerTimer?.cancel(); setState(() => _bannerMsg = null); },
...
class _ResvCard extends StatelessWidget {
  ...
  Color get _accent {
    switch (r.status) {
      case ReservationStatus.active:    return AppColors.green;
      case ReservationStatus.upcoming:  return AppColors.blue2;
      case ReservationStatus.completed: return AppColors.textMuted;
      case ReservationStatus.cancelled: return AppColors.red;
    }
  }
  String get _label {
    switch (r.status) {
      case ReservationStatus.active:    return 'En cours';
      case ReservationStatus.upcoming:  return 'À venir';
    ...
    return ParkCard(borderColor: _accent.withOpacity(0.25), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                builder: (dialogContext) => AlertDialog(
                  ...
                  actions: [
                    TextButton(onPressed: () => Navigator.of(dialogContext).pop(false),
                        child: const Text('Non', style: TextStyle(color: AppColors.textMuted))),
                    TextButton(onPressed: () => Navigator.of(dialogContext).pop(true),
                        child: const Text('Oui', style: TextStyle(color: AppColors.red))),
                  ]));
          ...
          Expanded(child: GestureDetector(
            ...
            child: Container(padding: const EdgeInsets.symmetric(vertical: 10),
              ...
              child: const Center(child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                Icon(Icons.more_time, color: Colors.white, size: 16), SizedBox(width: 5),
                Text('Prolonger', style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w600)),
              ]))))),
        ]),
      ],
      // QR Code — smart: Sortie si exitQrData موجود، Entrée sinon
      if (r.status == ReservationStatus.upcoming || r.status == ReservationStatus.active) ...[
        const SizedBox(height: 8),
        GestureDetector(onTap: () => _showQR(context),
          child: Container(width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 10),
            decoration: BoxDecoration(
              color: (r.exitQrData != null && r.exitQrData!.isNotEmpty) ? AppColors.orange.withOpacity(0.08) : AppColors.cyan.withOpacity(0.08),
              borderRadius: BorderRadius.circular(10),
              border: Border.all(color: (r.exitQrData != null && r.exitQrData!.isNotEmpty) ? AppColors.orange.withOpacity(0.3) : AppColors.cyan.withOpacity(0.3))),
      ...
      if (r.status == ReservationStatus.completed) ...[
        ...
        GestureDetector(
          ...
          child: Container(width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 10),
            decoration: BoxDecoration(color: AppColors.yellow.withOpacity(0.08), borderRadius: BorderRadius.circular(10),
                border: Border.all(color: AppColors.yellow.withOpacity(0.3))),
            child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              Icon(Icons.star_outline_rounded, color: AppColors.yellow, size: 18), SizedBox(width: 8),
      ...
      builder: (dialogContext) => StatefulBuilder(builder: (ctx, setS) => AlertDialog(
        ...
        content: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          ...[[30,'30 min'],[60,'1h'],[90,'1h30'],[120,'2h']].map((opt) {
            final mins = opt[0] as int; final label = opt[1] as String;
            final isSelected = selectedMinutes == mins;
            final cost = (mins / 60.0) * r.pricePerHour;
            return GestureDetector(onTap: () => setS(() => selectedMinutes = mins),
              child: Container(margin: const EdgeInsets.only(bottom: 8),
                ...
                child: Row(children: [
                  Text(label, style: TextStyle(fontWeight: FontWeight.w700,
                      color: isSelected ? AppColors.blue2 : AppColors.textPri)),
                  const Spacer(),
                  Text('+${cost.toStringAsFixed(2)} DT',
                      style: TextStyle(fontSize: 12, color: isSelected ? AppColors.cyan : AppColors.textMuted)),
                ])));
          }),
        ]),
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              try {
                final newEnd = await ref.read(prolongationServiceProvider).prolonger(
                  reservationId: r.id, extraMinutes: selectedMinutes, pricePerHour: r.pricePerHour);
                final updatedR = Reservation(id: r.id, userId: r.userId, userName: r.userName,
                  zoneId: r.zoneId, zoneName: r.zoneName, spotNumber: r.spotNumber,
                  vehiclePlate: r.vehiclePlate, startTime: r.startTime, endTime: newEnd,
                  pricePerHour: r.pricePerHour, status: r.status, createdAt: r.createdAt);
  ...
  void _showQR(BuildContext context) {
    final hasExitQr = r.exitQrData != null && r.exitQrData!.isNotEmpty;
    final qrData = hasExitQr
        ? r.exitQrData!
        : 'PARK|${r.id}|${r.zoneId}|${r.zoneName}|${r.spotNumber}'
          '|${r.startTime.toIso8601String()}'
          '|${(r.endTime ?? r.startTime.add(const Duration(hours: 1))).toIso8601String()}';
    final isExit   = hasExitQr;
    final title    = isExit ? 'QR Code de sortie' : "QR Code d'entrée";
    final subtitle = isExit ? 'Présentez ce QR à la sortie du parking' : "Présentez ce QR à l'entrée du parking";
    final color    = isExit ? AppColors.orange : AppColors.cyan;
      ...
      builder: (dialogCtx) => Dialog(
        ...
        child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            GestureDetector(onTap: () => Navigator.pop(dialogCtx),
          ...
          Container(
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              Icon(isExit ? Icons.logout : Icons.login, color: color, size: 14),
              const SizedBox(width: 6),
              Text(isExit ? 'Sortie' : 'Entrée',
          ...
          Text('Place ${r.spotNumber}', style: TextStyle(fontSize: 12, color: color)),
          const SizedBox(height: 6),
          Text('${DateFormat('dd/MM HH:mm').format(r.startTime)} → ${r.endTime != null ? DateFormat('HH:mm').format(r.endTime!) : '?'}',
              style: const TextStyle(fontSize: 11, color: AppColors.textMuted)),
          const SizedBox(height: 8),
      ...
      body: userAsync.when(
        data: (user) {
          ...
          return ListView(padding: const EdgeInsets.all(20), children: [
            ParkCard(bgColor: AppColors.blue.withOpacity(0.05), borderColor: AppColors.blue2.withOpacity(0.2),
              child: Row(children: [
...
class _AvatarWidget extends StatelessWidget {
  final AppUser user; final double size;
  const _AvatarWidget({required this.user, this.size = 58});
  @override
  Widget build(BuildContext context) => Container(
    width: size, height: size,
    decoration: const BoxDecoration(gradient: AppColors.blueGrad, shape: BoxShape.circle),
    child: Center(child: Text(user.name.isNotEmpty ? user.name[0].toUpperCase() : 'U',
...
class EditProfileScreen extends ConsumerStatefulWidget {
  ...
  @override ConsumerState<EditProfileScreen> createState() => _EditProfileState();
}
...
class _EditProfileState extends ConsumerState<EditProfileScreen> {
  ...
  @override
  void dispose() { _nameCtrl.dispose(); _phoneCtrl.dispose(); _plateCtrl.dispose(); _modelCtrl.dispose(); super.dispose(); }
  ...
  void _showImagePicker() {
    showModalBottomSheet(context: context, backgroundColor: AppColors.surface,
      ...
      builder: (_) => Padding(padding: const EdgeInsets.all(20), child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(height: 16),
        Row(children: [
          Expanded(child: GestureDetector(onTap: () { Navigator.pop(context); _pickImage(ImageSource.camera); },
            child: Container(padding: const EdgeInsets.symmetric(vertical: 16),
              decoration: BoxDecoration(color: AppColors.blue2.withOpacity(0.08), borderRadius: BorderRadius.circular(14),
                  border: Border.all(color: AppColors.blue2.withOpacity(0.2))),
              child: const Column(children: [Icon(Icons.camera_alt_outlined, color: AppColors.blue2, size: 28),
                SizedBox(height: 8), Text('Caméra', style: TextStyle(color: AppColors.blue2, fontWeight: FontWeight.w600))])))),
          const SizedBox(width: 12),
          Expanded(child: GestureDetector(onTap: () { Navigator.pop(context); _pickImage(ImageSource.gallery); },
            child: Container(padding: const EdgeInsets.symmetric(vertical: 16),
              decoration: BoxDecoration(color: AppColors.green.withOpacity(0.08), borderRadius: BorderRadius.circular(14),
                  border: Border.all(color: AppColors.green.withOpacity(0.2))),
              child: const Column(children: [Icon(Icons.photo_library_outlined, color: AppColors.green, size: 28),
                SizedBox(height: 8), Text('Galerie', style: TextStyle(color: AppColors.green, fontWeight: FontWeight.w600))])))),
        ]),
      ])));
  }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (_pickedImage != null) {
        try {
          final ref2 = FirebaseStorage.instance.ref('profiles/${user.uid}.jpg');
          await ref2.putData(await _pickedImage!.readAsBytes());
          photoUrl = await ref2.getDownloadURL();
        } catch (_) {}
      }
      final data = <String, dynamic>{
        'name': _nameCtrl.text.trim(), 'phone': _phoneCtrl.text.trim(),
        'vehiclePlate': _plateCtrl.text.trim(), 'vehicleModel': _modelCtrl.text.trim(),
      };
        ...
        Center(child: GestureDetector(onTap: _showImagePicker,
          child: Stack(children: [
            Container(width: 100, height: 100,
              decoration: BoxDecoration(gradient: _pickedImage == null ? AppColors.blueGrad : null,
                shape: BoxShape.circle, border: Border.all(color: AppColors.blue2.withOpacity(0.3), width: 3)),
              child: _pickedImage != null
                  ? ClipOval(child: Image.file(File(_pickedImage!.path), fit: BoxFit.cover, width: 100, height: 100))
                  : Center(child: Text(_nameCtrl.text.isNotEmpty ? _nameCtrl.text[0].toUpperCase() : 'U',
                      style: const TextStyle(fontSize: 36, fontWeight: FontWeight.w800, color: Colors.white)))),
            Positioned(bottom: 0, right: 0, child: Container(width: 32, height: 32,
              decoration: BoxDecoration(color: AppColors.blue2, shape: BoxShape.circle,
                  border: Border.all(color: Colors.white, width: 2)),
              child: const Icon(Icons.camera_alt, color: Colors.white, size: 16))),
          ]))),
        const SizedBox(height: 8),
        const Center(child: Text('Appuyez pour changer la photo', style: TextStyle(fontSize: 11, color: AppColors.textMuted))),
        const SizedBox(height: 24),
        const Text('INFORMATIONS PERSONNELLES', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: AppColors.textMuted, letterSpacing: 0.8)),
        const SizedBox(height: 10),
...
class AvisScreen extends ConsumerStatefulWidget {
  final String zoneId, zoneName;
  const AvisScreen({super.key, required this.zoneId, required this.zoneName});
  @override ConsumerState<AvisScreen> createState() => _AvisState();
}
...
class _AvisState extends ConsumerState<AvisScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      await ref.read(parkingServiceProvider).submitAvis(
        ...
        userId: user?.uid ?? '', userName: user?.name ?? 'Anonyme',
  ...
  Widget _buildSuccess() => Center(child: Padding(padding: const EdgeInsets.all(32),
    child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      ...
      const SizedBox(height: 20),
      const Text('Merci pour votre avis !', style: TextStyle(fontSize: 22, fontWeight: FontWeight.w800)),
      const SizedBox(height: 8),
      Text('Votre avis sur ${widget.zoneName} a été enregistré.',
          style: const TextStyle(fontSize: 13, color: AppColors.textMuted), textAlign: TextAlign.center),
      const SizedBox(height: 32),
      ParkButton(label: 'Retour à l\'accueil', icon: Icons.home_outlined, onTap: () => context.go('/client/home')),
    ])));

  Widget _buildForm() => ListView(padding: const EdgeInsets.all(24), children: [
    ParkCard(bgColor: AppColors.blue.withOpacity(0.07), borderColor: AppColors.blue2.withOpacity(0.2),
      child: Row(children: [
        Container(width: 44, height: 44,
          decoration: BoxDecoration(gradient: AppColors.blueGrad, borderRadius: BorderRadius.circular(12)),
          child: const Icon(Icons.local_parking, color: Colors.white, size: 22)),
        const SizedBox(width: 14),
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Text('Évaluer votre expérience', style: TextStyle(fontSize: 12, color: AppColors.textMuted)),
          Text(widget.zoneName, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
        ])),
      ])),
    const SizedBox(height: 28),
    const Text('NOTE', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: AppColors.textMuted, letterSpacing: 0.8)),
    const SizedBox(height: 14),
    Row(mainAxisAlignment: MainAxisAlignment.center, children: List.generate(5, (i) {
      final filled = i < _rating;
      return GestureDetector(onTap: () => setState(() => _rating = i + 1),
        child: AnimatedContainer(duration: const Duration(milliseconds: 150),
          margin: const EdgeInsets.symmetric(horizontal: 6),
          child: Icon(filled ? Icons.star_rounded : Icons.star_outline_rounded,
              color: filled ? AppColors.yellow : AppColors.textMuted, size: filled ? 44 : 38)));
    })),
    const SizedBox(height: 8),
    Center(child: Text(
    ...
    const SizedBox(height: 28),
    const Text('COMMENTAIRE (optionnel)', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: AppColors.textMuted, letterSpacing: 0.8)),
    const SizedBox(height: 10),
    Container(
      decoration: BoxDecoration(color: AppColors.card, borderRadius: BorderRadius.circular(14), border: Border.all(color: AppColors.border)),
      child: TextField(controller: _commentCtrl, maxLines: 4, style: const TextStyle(fontSize: 13),
