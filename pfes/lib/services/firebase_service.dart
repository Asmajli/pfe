...
class ParkingService {
  ...
  Stream<List<Reservation>> userReservations(String uid) =>
      _db.collection('reservations').where('userId', isEqualTo: uid)
  ...
  Stream<List<Reservation>> zoneActiveReservations(String zoneId) =>
      _db.collection('reservations').where('zoneId', isEqualTo: zoneId)
          .where('status', whereIn: ['active', 'upcoming'])
          .orderBy('startTime')
          .snapshots().map((s) => s.docs.map(Reservation.fromDoc).toList());
  ...
  Stream<List<VehicleLog>> zoneLogs(String zoneId) =>
      _db.collection('vehicle_logs').where('zoneId', isEqualTo: zoneId)
          .orderBy('timestamp', descending: true).limit(50)
          .snapshots().map((s) => s.docs.map(VehicleLog.fromDoc).toList());
...
final parkingServiceProvider = Provider((_) => ParkingService());
...
final currentUserProvider = StreamProvider<AppUser?>((ref) async* {
  ...
  await for (final fu in auth.authStateChanges()) {
    ...
    final doc = await db.collection('users').doc(fu.uid).get();
    yield doc.exists ? AppUser.fromMap(fu.uid, doc.data()!) : null;
  }
});
